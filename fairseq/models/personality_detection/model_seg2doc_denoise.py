...
@register_model("seg2doc_denoise")
class PDModel(FairseqEncoderModel):
    def __init__(self, args, sen_encoder, trait, vote):
    ...
    def build_model(cls, dictionary):
        ...
        encoder = RobertaEncoder(args, dictionary)

        return cls(args, encoder, None, False)
    ...
    def forward(
        ...
        **kwargs
        ...
        for mairesse, senticnet, emotion in zip(kwargs['seg_mairesse'], kwargs['seg_senticnet'], kwargs['seg_emotion']):
            seg_features.append(torch.concatenate([mairesse, senticnet, emotion], dim=-1))
        ...
        for i, seg in enumerate(src_tokens):  # batch_size_2(seg_num) * seq_length
            ...
            doc_rep = self.out_proj(attn)   # 6 * 128
            outputs.append(torch.flatten(doc_rep))  # 768

        outputs = torch.stack(outputs, dim=0)   # batch_size * 768
...
@register_model_architecture("seg2doc_denoise", "seg2doc_denoise")
