...
from fairseq.tokenizer import tokenize_line, tokenize_doc
...
class Dictionary:
    ...
    def encode_doc(
        ...
        doc_sent_words = doc_tokenizer(doc_text)
        word_size = max(len(sent_words) for sent_words in doc_sent_words)
        sent_size = len(doc_sent_words)
        ids = torch.full([sent_size, word_size + 1 if append_eos else word_size],
                         self.pad_index, dtype=torch.int)
        for i, sent_words in enumerate(doc_sent_words):
