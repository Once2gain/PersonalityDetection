...
def make_dataset(path, impl, fix_lua_indexing=False, dictionary=None):
    ...
    elif impl == "doc" and DocIndexedDataset.exists(path):
        return DocIndexedDataset(path)
...
class DocIndexedDataset(torch.utils.data.Dataset):
    class Index:
        _HDR_MAGIC = b"MMIDIDX\x00\x00"
        ...
        for it in np_array:
            ...
            if end:
                if it == 1:
                    ...
                    break
        assert size % inner_size == 0, "size % inner_size == 0"
        np_array = np_array.reshape(-1, inner_size)
        assert inner_size < 512, "inner_size < 512"
